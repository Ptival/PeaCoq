...
function prBinder(
    many : boolean,
    pr : (c : ConstrExpr) => Pp.t,
    [nal, k, t] : [cAST<Name>[], BinderKind, ConstrExpr]
) : Pp.t {
...
function printHunks(
    n : number,
    pr : (_1 : [number, ParenRelation], _2 : ConstrExpr) => Pp.t,
    prPatt : (_1 : [number, ParenRelation], _2 : CasesPatternExpr) => Pp.t,
...
function prProj(
    pr : (_1 : ConstrExpr.ConstrExprR, _2 : ConstrExpr) => Pp.t,
    prApp : (
        pr : (_1 : PrecAssoc, _2 : ConstrExpr) => Pp.t,
        a : ConstrExpr,
...
function prExplArgs(
    pr : (pa : PrecAssoc, ce : ConstrExpr) => Pp.t,
...
function tagConstrExpr(ce : ConstrExpr, cmds : Pp.t) {
        ...
        if (pp instanceof CPatCstr) {
            ...
            if (
                ...
                const [strmNot, lNot] = prNotation(
                    (x, y : CasesPatternExpr) => prPatt(Pp.mt, x, y),
                    (x : any, y : any) => Pp.mt(),
...
function prCaseItem(
    pr : (_1 : PrecAssoc, _2 : ConstrExpr) => Pp.t,
    [tm, asClause, inClause] : [ConstrExpr, Maybe<cAST<Name>>, Maybe<ConstrExpr>]
) : Pp.t {
...
function constrLoc(c : ConstrExpr) : Maybe<Loc.t> {
    return c.loc
}
...
function prGen(
    pr : (_1 : () => Pp.t, _2 : PrecAssoc, _3 : ConstrExpr) => Pp.t
) : (_1 : () => Pp.t, _2 : PrecAssoc, _3 : ConstrExpr) => Pp.t {
    return (
        sep : () => Pp.t,
        inherited : PrecAssoc,
        a : ConstrExpr
        ...
        const prmt = (x : [number, ParenRelation], y : ConstrExpr) => pr(Pp.mt, x, y)
        ...
        function match(aa : ConstrExpr.ConstrExprR) : PpResult {
            ...
            if (aa instanceof ConstrExpr.CCases) {
                ...
                const prDangling = (pa : [number, ParenRelation], c : ConstrExpr) => prDanglingWithFor(Pp.mt, pr, pa, c)
                return ret(
                    Pp.v(0,
            ...
            if (aa instanceof ConstrExpr.CLambdaN) {
                const [bl, a1] = ConstrExpr.extractLamBinders(aa)
                return ret(
                    Pp.hov(0, ([] as Pp.t[]).concat(
                        Pp.hov(2, prDelimitedBinders(prFun, Pp.spc, x => pr(Pp.spc, lTop, x), bl)),
                        prFunSep,
                        pr(Pp.spc, lTop, a1)
                    )),
                    lLambda
            ...
            if (aa instanceof ConstrExpr.CNotation) {
                if (aa.notation === '(\u00A0_\u00A0)') {
                    ...
                    return prNotation(
                        (x : [number, ParenRelation], y : {}) => pr(Pp.mt, x, y),
            ...
            if (aa instanceof ConstrExpr.CProdN) {
                const [bl, aRest] = ConstrExpr.extractProdBinders(aa)
                return ret(
                    ([] as Pp.t[]).concat(
                        prDelimitedBinders(
                            prForall,
...
export function prConstrExpr(a : ConstrExpr) : Pp.t {
...
function prHTMLGen(
    ...
    return (sep : () => Pp.t, pa : PrecAssoc, e : ConstrExpr) => {
        return ([] as Pp.t[]).concat(
...
function prHTML(a : ConstrExpr) : Pp.t {
...
// function ppCmdSameShape(p : PpExtend.PpCmd, old : PpExtend.PpCmd) : boolean {
